  ...
  return (
    <div
      className={`bg-slate dark:bg-black dark:text-white border border-slate dark:border-white p-2 md:p-4 ${
        !canLeaveReview ? "md:rounded-full" : "md:rounded-lg"
      }`}
    >
      <div className="flex justify-between">
        <div className="flex gap-3">
          ...
          <div className="flex flex-col">
            <span className="font-bold">{review.source} Reviews</span>
            {review.average_rating && (
              <span className="flex flex-wrap items-center gap-4">
                <span>({review.average_rating})</span>
                <Rating
                  initialRating={review.average_rating}
                  readonly={true}
                  emptySymbol={
                    <svg
                      className={"w-6 h-6"}
        ...
        <div>{review.review_count} reviews</div>
      </div>
